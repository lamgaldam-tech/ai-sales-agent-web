...
export default function CustomersPage() {
  ...
  async function handleSendMessage() {
    ...
    const { data } = await supabase
      .from('messages')
      .insert({ customer_id: selected.id, content, role: 'assistant' })
      .select('*')
      .single()
    ...
    <div>
      ...
      {customers.length === 0 ? (
        <EmptyState
          ...
          description="Customers will appear here once they start chatting with your AI agent on WhatsApp"
        />
      ) : (
                ...
                {filtered.map((c) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-gray-400">{formatPhone(c.phone)}</p>
                    </div>
            ...
            {selected ? (
              <div className="flex flex-1 flex-col">
                <div className="flex items-center gap-3 border-b border-gray-200 px-4 py-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{selected.name || 'Unknown'}</p>
                  ...
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      ...
                      >
                        ...
                        >
                          {msg.content}
                          <p className={`mt-1 text-xs ${msg.role === 'assistant' ? 'text-primary-200' : 'text-gray-400'}`}>
              ...
              <div className="hidden flex-1 flex-col items-center justify-center text-center md:flex">
                ...
                <p className="text-sm text-gray-400">Select a customer to view conversation</p>
              </div>
            )}
          </div>
        </div>
