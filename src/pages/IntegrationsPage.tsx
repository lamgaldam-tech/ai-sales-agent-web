...
const OAUTH_SCOPES: Record<IntegrationType, string> = {
  shopify: 'read_products,read_orders',
  youcan: 'read_products,read_orders',
...
export default function IntegrationsPage() {
  ...
  async function fetchIntegrations() {
    ...
    try {
      ...
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/api/integrations`
  ...
  async function handleConnect() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Failed to start connection')
      }
    } catch {
            ...
            return (
              <div key={int.id} className="card p-5">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <span className={`text-xs font-medium ${int.connected ? 'text-accent-600' : 'text-gray-400'}`}>
                    {int.connected ? 'Connected' : 'Not connected'}
                  </span>
                  <button
                    onClick={() => handleDisconnect(int.id)}
                    className="rounded-lg p-2 text-gray-400 hover:bg-error-50 hover:text-error-600 transition-colors"
                    title="Remove"
                  >
            ...
            <div className="mt-5 space-y-4">
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center gap-2 rounded-lg border-2 p-3 transition-all ${
                          newType === type ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
              ...
              <div>
                <label className="label">
                  {newType === 'google_sheets' ? 'Spreadsheet ID' : 'Shop Domain'}
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="input"
                  placeholder={newType === 'google_sheets' ? '1ABC...xyz' : 'mystore.myshopify.com'}
                />
              </div>
            </div>
