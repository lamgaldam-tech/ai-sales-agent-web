...
export default function SettingsPage() {
  ...
  async function handleSavePrompt() {
    ...
    if (promptId) { await supabase.from('prompts').update({ content: prompt }).eq('id', promptId) }
    else { const { data } = await supabase.from('prompts').insert({ business_id: business.id, content: prompt }).select('*').single(); if (data) setPromptId(data.id) }
  ...
  if (loading) {
    return (<div><PageHeader title="Settings" description="Manage your business profile and AI configuration" /><div className="flex h-64 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary-600" /></div></div>)
  }
