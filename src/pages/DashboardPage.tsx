...
import type { Customer, Order, Message } from '../lib/types'
...
export default function DashboardPage() {
  ...
  const [recentMessages, setRecentMessages] = useState<(Message & { customer: Customer | null })[]>([])
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [customersRes, ordersRes, messagesRes] = await Promise.all([
        ...
        supabase.from('orders').select('*, customer:customers(*)').eq('customer.business_id', business.id).order('created_at', { ascending: false }),
        supabase.from('messages').select('*, customer:customers(*)').eq('customer.business_id', business.id).order('created_at', { ascending: false }).limit(10),
      ])

      const customers = customersRes.data as Customer[] || []
      const orders = (ordersRes.data as (Order & { customer: Customer | null })[]) || []
      const messages = (messagesRes.data as (Message & { customer: Customer | null })[]) || []
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          label="Revenue"
          value={formatCurrency(stats.revenue, business?.currency)}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/customers" className="text-xs font-medium text-primary-600 hover:text-primary-700 flex items-center gap-1">
          ...
          {recentCustomers.length === 0 ? (
            <EmptyState
              ...
              description="Customers will appear here once they start chatting with your AI agent"
            />
          ) : (
        ...
        <div className="card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/orders" className="text-xs font-medium text-primary-600 hover:text-primary-700 flex items-center gap-1">
              View all <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
